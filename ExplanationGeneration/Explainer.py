...
class Explainer:
    ...
    def __init__(self,
        ...
        self.mutual_information_manager = MutualInformationManager(
            linkage_table_file=mutual_information_linkage_table_file,
    ...
    def explain_solution(self, solution: EvaluatedFS, shown_ps_max: int, must_contain: Optional[int] = None):
        ...
        contained_pss.sort(key=lambda x: len(x.explanations), reverse=True)  # sort by amount of explanations
    ...
    def handle_plotvar_query(self):
        ...
        def repr_var_nicely(varname):
            pretty_name = self.problem.get_readable_property_name(varname)
            if pretty_name == varname:
                return varname
            else:
                return f"{varname} = {pretty_name}"

        print("\tOptions for properties are " + ", ".join(
            "\n\t\t" + repr_var_nicely(varname) for varname in self.ps_property_manager.get_available_properties()))
    ...
    def generate_files_with_default_settings(self,
        ...
        self.ps_property_manager.generate_property_table_file(self.mined_ps_manager.pss,
                                                              self.mined_ps_manager.control_pss)
    ...
    def handle_plotprop_query(self):
        print("\tOptions for properties are " + ", ".join(
            varname for varname in self.ps_property_manager.get_available_properties()))
        property_name = input("Which property? ")
