...
def sharing_value_between_PS_values(vals_a: np.ndarray, vals_b: np.ndarray, sigma_shared: float, alpha: int) -> float:
    distance = distance_between_PS_values(vals_a, vals_b)
...
def get_sharing_scores(solution_matrix: np.ndarray, sigma_shared: float, alpha: int) -> ArrayOfFloats:
    ...
    for row_index_a in range(solution_amount):
        ...
        for row_index_b in range(row_index_a+1, solution_amount):
            ...
            shared_value_matrix[row_index_a][row_index_b] = sharing_value_between_PS_values(row_a, row_b,
                                                                                            sigma_shared, alpha)
